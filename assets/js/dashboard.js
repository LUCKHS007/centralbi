...
// Logos de marca (não dá pra usar Font Awesome pra essas) — SVG mono com
// fill="currentColor" pra herdar a cor de acento do tema selecionado,
// igual os ícones normais. `item.icone` vem como "logo:grafana" etc.
const LOGOS_DE_MARCA = {
...
function criarIconeMarca(nomeLogo){

    const svgNS = "http://www.w3.org/2000/svg";
    const svg = document.createElementNS(svgNS, "svg");
    svg.setAttribute("viewBox", "0 0 24 24");
    svg.classList.add("icone-marca");

    const path = document.createElementNS(svgNS, "path");
    path.setAttribute("d", LOGOS_DE_MARCA[nomeLogo]);
...
function criarCardBi(item, { href, categoria, textoAbrir }){
    ...
    if (item.icone && item.icone.startsWith("logo:")){

        card.appendChild(criarIconeMarca(item.icone.slice(5)));
